      ...
      private
        ...
        def build!
          in_working_directory(base_dir) do
            ...
            Docker::Image.build_from_dir(base_dir, build_opts) { |out| $logger.debug '%s' % [ out.strip ] }
          end
        end

        def execute_pre_build_scripts!
          ExecLocal.new(container, container.pre_build_scripts, 'PRE build').execute! unless container.pre_build_scripts.empty?
        end
